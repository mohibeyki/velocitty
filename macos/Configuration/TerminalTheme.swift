...
public enum TerminalTheme {
  ...
  private static func resolveOptions(
    ...
    if requested.hasPrefix("light:") || requested.hasPrefix("dark:") {
      ...
      for part in requested.split(separator: ",") {
        ...
        guard pair.count == 2, ["light", "dark"].contains(pair[0]), !pair[1].isEmpty,
          choices[pair[0]] == nil
        else {
      ...
      guard choices.count == 2 else {
        throw ConfigurationError("theme requires both light and dark variants.")
      }
    ...
    if name.hasPrefix("/") || name.hasPrefix("~/") {
      ...
      custom = source.deletingLastPathComponent().appendingPathComponent("themes")
  ...
  public static func decode(_ data: Data, source: URL) throws -> [TerminalOption] {
    ...
    var mapping = [
      ...
      "Cursor Text Color": "cursor-text", "Selection Color": "selection-background",
      "Selected Text Color": "selection-foreground", "Bold Color": "bold-color",
    ]
    ...
    for (label, key) in mapping.sorted(by: { $0.key < $1.key }) {
      ...
      guard let components = entry as? [String: Any] else {
        throw ConfigurationError("\(source.path): invalid color: \(label)")
      }
      let channels = ["Red Component", "Green Component", "Blue Component"].compactMap {
        components[$0] as? Double
      }
      ...
      let color: NSColor
      switch components["Color Space"] as? String ?? "Calibrated" {
      case "sRGB":
        color = NSColor(srgbRed: channels[0], green: channels[1], blue: channels[2], alpha: 1)
      case "P3", "Display P3":
        color = NSColor(displayP3Red: channels[0], green: channels[1], blue: channels[2], alpha: 1)
      case "Calibrated":
        color = NSColor(calibratedRed: channels[0], green: channels[1], blue: channels[2], alpha: 1)
      default:
        throw ConfigurationError("\(source.path): unsupported color space for \(label)")
      }
      guard let rgb = color.usingColorSpace(.sRGB) else {
        throw ConfigurationError("\(source.path): could not convert color \(label)")
      }
      let bytes = [rgb.redComponent, rgb.greenComponent, rgb.blueComponent].map {
        Int((min(1, max(0, $0)) * 255).rounded())
      }
    ...
    guard result.contains(where: { $0.key == "background" }),
      result.contains(where: { $0.key == "foreground" })
    else {
      throw ConfigurationError(
        "\(source.path): a theme must define background and foreground colors.")
    }
    return result
  }
}
