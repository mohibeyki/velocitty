...
final class CommandPalette: NSPanel, NSSearchFieldDelegate, NSTableViewDataSource, NSTableViewDelegate {
    struct Entry { let title: String; let detail: String; let action: String }
    ...
    static let unsupported: Set<String> = [
        "new_tab", "new_split", "goto_tab", "goto_split", "move_tab", "move_tab_to_new_window",
        "resize_split", "equalize_splits", "toggle_split_zoom", "toggle_tab_overview", "toggle_quick_terminal",
        "undo", "redo", "check_for_updates", "inspector", "show_gtk_inspector", "crash", "write_screen_file",
        "write_scrollback_file", "write_selection_file", "export_terminal_io", "toggle_command_palette"
    ]
    init(terminal: TerminalView) {
        self.terminal = terminal
        super.init(contentRect: NSRect(x: 0, y: 0, width: 620, height: 420), styleMask: [.titled, .closable], backing: .buffered, defer: false)
        title = "Commands"
        isReleasedWhenClosed = false
        let native = NativeSettings(config: terminal.config)
        let commands = native.value("command-palette-entry", ghostty_config_command_list_s(commands: nil, len: 0))
        if let pointer = commands.commands {
            entries = (0..<commands.len).compactMap { index in
                ...
                guard let key = command.action_key, !Self.unsupported.contains(String(cString: key)),
                      let action = command.action, let title = command.title else { return nil }
                return Entry(title: String(cString: title), detail: command.description.map { String(cString: $0) } ?? "", action: String(cString: action))
            }
        }
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        let text = NSTextField(labelWithString: entry.title + (entry.detail.isEmpty ? "" : "\n" + entry.detail))
    ...
    func filter() {
        ...
        filtered = entries.filter { needle.isEmpty || $0.title.localizedCaseInsensitiveContains(needle) || $0.detail.localizedCaseInsensitiveContains(needle) }
        table.reloadData()
    ...
    func control(_ control: NSControl, textView: NSTextView, doCommandBy commandSelector: Selector) -> Bool {
        ...
        if commandSelector == #selector(NSResponder.moveDown(_:)) || commandSelector == #selector(NSResponder.moveUp(_:)) {
            ...
            if row >= 0 { table.selectRowIndexes(IndexSet(integer: row), byExtendingSelection: false); table.scrollRowToVisible(row) }
            return true
        }
        return false
    }
