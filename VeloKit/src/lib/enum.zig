...
/// `velokit.h` with the correct value. This should only ever be called inside a `test`
/// because the `velokit.h` module is only available then.
pub fn checkGhosttyHEnum(
    ...
    std.testing.expect(set.count() == 0) catch |e| {
        ...
        while (it.next()) |v| {
            ...
            std.log.err("velokit.h is missing value for {s}{s}", .{ prefix, upper_string });
        }
        return e;
    };
}
