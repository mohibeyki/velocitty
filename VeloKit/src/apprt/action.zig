...
pub const Action = union(Key) {
    ...
    //
    // 4. Update `include/velokit.h`: add the new key, value, and union
    ...
    previous_namespace,
    next_namespace,
    goto_namespace: NamespaceIndex,
    rename_namespace,

    pub const NamespaceIndex = extern struct { index: u32 };
...
pub const ResizeSplit = extern struct {
    ...
    pub const Direction = enum(c_int) {
        ...
        test "velokit.h ResizeSplit.Direction" {
...
pub const MouseVisibility = enum(c_int) {
    ...
    test "velokit.h MouseVisibility" {
...
/// sync with ghostty_action_close_tab_mode_e in velokit.h
...
pub const CommandFinished = struct {
    ...
    /// sync with ghostty_action_command_finished_s in velokit.h
    pub const C = extern struct {
        exit_code: i16,
        duration: u64,
    };
