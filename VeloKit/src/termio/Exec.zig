...
const Subprocess = struct {
    ...
    pub fn init(gpa: Allocator, cfg: Config) !Subprocess {
        ...
        if (cfg.resources_dir) |dir| {
            log.info("found terminal resources dir: {s}", .{dir});
        ...
        if (cfg.resources_dir) |base| {
            ...
            if (comptime builtin.target.os.tag.isDarwin()) {
                log.warn("bundled terminfo not found, using xterm-256color", .{});
        ...
        ghostty_path: {
            ...
            const exe_bin_path = exe_buf[0 .. std.process.executablePath(
                ...
                log.warn("failed to get application executable path err={}", .{err});
                break :ghostty_path;
            }];
            const exe_dir = std.fs.path.dirname(exe_bin_path) orelse break :ghostty_path;
            log.debug("appending application bin to path dir={s}", .{exe_dir});
