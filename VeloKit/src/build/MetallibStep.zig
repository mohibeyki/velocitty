...
pub fn create(b: *std.Build, opts: Options) ?*MetallibStep {
    ...
    // The Metal compiler is an optional Xcode component. On recent macOS
    // releases it is provided through cryptex and is discoverable by xcrun
    // without an SDK qualifier; asking for `-sdk macosx` selects Xcode's
    // placeholder binary instead and reports a missing toolchain.
    run_ir.addArgs(&.{ "/usr/bin/xcrun", "metal", "-o" });
    ...
    run_lib.addArgs(&.{ "/usr/bin/xcrun", "metallib", "-o" });
