// Private Swift/Zig bridge for the Velocitty application.
// Derived from libghostty's embedding interface; not a supported external SDK.
// Shared C data layouts must match their Zig counterparts.
#ifndef VELOKIT_H
#define VELOKIT_H
...
ghostty_config_t velokit_config_clone(ghostty_config_t);
...
ghostty_surface_t velokit_surface_new(ghostty_app_t,
                                       const ghostty_surface_config_s*);
void velokit_surface_free(ghostty_surface_t);
void* velokit_surface_userdata(ghostty_surface_t);

void velokit_surface_draw(ghostty_surface_t);
void velokit_surface_set_content_scale(ghostty_surface_t, double, double);
void velokit_surface_set_focus(ghostty_surface_t, bool);

void velokit_surface_set_size(ghostty_surface_t, uint32_t, uint32_t);

bool velokit_surface_key(ghostty_surface_t, ghostty_input_key_s);

void velokit_surface_preedit(ghostty_surface_t, const char*, uintptr_t);

bool velokit_surface_binding_action(ghostty_surface_t, const char*, uintptr_t);
void velokit_surface_complete_clipboard_request(
    ghostty_surface_t,
    const ghostty_clipboard_complete_s*,
    void*);
void velokit_surface_deny_clipboard_request(ghostty_surface_t,
                                           void*);
